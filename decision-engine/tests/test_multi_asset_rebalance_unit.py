...
def test_target_rejects_weights_not_summing_to_one() -> None:
    with pytest.raises(ValueError):
        MultiAssetTarget(
            weights={"USDC": Decimal("0.4"), "ETH": Decimal("0.4"), "WBTC": Decimal("0.4")},
            band=Decimal("0.10"),
            hub="USDC",
        )
...
def test_target_rejects_non_positive_weight() -> None:
    with pytest.raises(ValueError):
        MultiAssetTarget(
            weights={"USDC": Decimal("0.5"), "ETH": Decimal("0.5"), "WBTC": Decimal("0")},
            band=Decimal("0.10"),
            hub="USDC",
        )
...
def test_target_rejects_hub_not_in_weights() -> None:
    with pytest.raises(ValueError):
        MultiAssetTarget(
            weights={"USDC": Decimal("0.4"), "ETH": Decimal("0.32"), "WBTC": Decimal("0.28")},
            band=Decimal("0.10"),
            hub="DAI",
        )
...
def test_target_accepts_valid_construction() -> None:
    ...
    assert _TARGET.weights["ETH"] == Decimal("0.32")
...
def test_within_band_holds() -> None:
    ...
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("4000"), "ETH": Decimal("3200"), "WBTC": Decimal("2800")},
        target=_TARGET,
...
def test_empty_portfolio_holds() -> None:
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("0"), "ETH": Decimal("0"), "WBTC": Decimal("0")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )
...
def test_one_crypto_overweight_sells_to_hub() -> None:
    ...
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("3350"), "ETH": Decimal("4500"), "WBTC": Decimal("2150")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )
    assert plan.action == "rebalance"
    assert plan.from_symbol == "ETH"
...
def test_one_crypto_underweight_buys_from_hub() -> None:
    ...
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("4650"), "ETH": Decimal("1900"), "WBTC": Decimal("3450")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )
...
def test_selects_single_largest_drift_when_two_out_of_band() -> None:
    ...
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("3800"), "ETH": Decimal("4600"), "WBTC": Decimal("1600")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )
...
def test_hub_overweight_buys_most_underweight_crypto() -> None:
    ...
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("5200"), "ETH": Decimal("2400"), "WBTC": Decimal("2400")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )
...
def test_hub_underweight_sells_most_overweight_crypto() -> None:
    ...
    # ETH excess $800) → $800.
    plan = plan_multi_rebalance(
        holdings={"USDC": Decimal("2800"), "ETH": Decimal("4000"), "WBTC": Decimal("3200")},
        target=_TARGET,
        est_cost_usd=_NO_COST,
        cost_gate_margin=_MARGIN,
    )

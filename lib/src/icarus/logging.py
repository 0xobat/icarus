...
Ported verbatim from v4.2 (.archive/py-engine/monitoring/logger.py) during
the W1D4 validated copy-back. The risk modules in
`decision-engine/risk/` use this. Future convergence with `structlog` (used
by the service `__main__` entrypoints) is a follow-up — for now the two
loggers coexist.

Stability rule: this module's public API (`get_logger`, `correlation_context`,
`get_correlation_id`) is consumed by every risk module. Changing it
requires updating all consumers + their tests.
"""
...
@contextmanager
def correlation_context(correlation_id: str | None = None) -> Generator[str]:

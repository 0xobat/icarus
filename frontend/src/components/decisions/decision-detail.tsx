    ...
    >
            ...
            <div key={report.strategy_id} className="flex items-start gap-2">
              ...
              <div className="flex flex-wrap gap-1">
                {report.signals.map((signal) => (
                  <span
                    key={signal}
                    className="rounded bg-bg-elevated px-1.5 py-0.5 font-mono text-[8px] text-text-secondary"
                  >
                    {signal}
                  </span>
                ))}
      ...
      {isCB ? (
        <div className="border-b border-border-subtle px-4 py-3">
          ...
          <p className="mt-2 text-xs text-text-secondary">{decision.reasoning}</p>
        </div>
      ) : (
        <div className="border-b border-l-2 border-b-border-subtle border-l-cyan/30 px-4 py-3">
          ...
          <p className="mt-2 text-xs leading-relaxed text-text-secondary">{decision.reasoning}</p>
        </div>
      )}
            ...
            {decision.orders.map((order, i) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="rounded bg-primary-muted px-1.5 py-0.5 font-mono text-[8px] uppercase text-primary">
                    {order.action}
                  </span>
                  <span className="font-mono text-[10px] text-text-primary">
                    {order.protocol}
                  </span>
                  <span className="font-mono text-[10px] text-text-tertiary">{order.asset}</span>
                </div>
        ...
        <div className="border-b border-border-subtle px-4 py-3">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1">
            {decision.verification.checks.map((check) => (
              <span
                key={check}
                className="rounded bg-success-muted px-1.5 py-0.5 font-mono text-[8px] text-success"
              >
                {check}
              </span>
            ))}
